...
public interface ClassBookingRepository extends JpaRepository<ClassBooking, UUID> {
    ...
    List<ClassBooking> findByClassSessionId(UUID sessionId);

    List<ClassBooking> findByUserId(UUID userId);
    ...
    // Calls the book_class_slot() stored function — performs duplicate, conflict,
    // and capacity checks atomically then inserts the booking.
    @Query(value = "SELECT book_class_slot(CAST(:userId AS uuid), CAST(:sessionId AS uuid))", nativeQuery = true)
    String callBookClassSlot(@Param("userId") String userId, @Param("sessionId") String sessionId);
}