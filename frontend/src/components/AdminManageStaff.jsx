...
export default function AdminManageStaff() {
    ...
    // Rate state per staff
    const [editingRate, setEditingRate] = useState(null);
    const [newRate, setNewRate] = useState('');
    const [savingRate, setSavingRate] = useState(false);
    const [rateStatus, setRateStatus] = useState({ userId: null, type: '', text: '' });
    ...
    const handleSaveRate = async (userId) => {
        setSavingRate(true);
        try {
            await api.patch(`/api/admin/users/${userId}/hourly-rate`, {
                hourlyRate: parseFloat(newRate) || 0
            });
            setRateStatus({ userId, type: 'success', text: 'Saved.' });
            setEditingRate(null);
            setNewRate('');
            fetchStaff();
            setTimeout(() => setRateStatus({ userId: null, type: '', text: '' }), 3000);
        } catch (err) {
            setRateStatus({ userId, type: 'error', text: 'Failed.' });
        } finally {
            setSavingRate(false);
        }
    };
        ...
        <div className="p-6 md:p-10 max-w-6xl mx-auto h-full overflow-y-auto">
            <div className="mb-8">
                ...
                <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
                    Create staff accounts and manage hourly pay via QR attendance. To manage trainer pay rates, go to <span className="text-olive dark:text-lightSage font-bold">Trainer Directory</span>.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white dark:bg-darkCard border border-gray-100 dark:border-gray-800 rounded-3xl shadow-lg shadow-gray-200/50 dark:shadow-none overflow-hidden h-fit">
                    <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-darkBg/50 flex items-center justify-between">
                        <h2 className="font-bold text-gray-900 dark:text-cream text-lg tracking-tight">Create Staff Account</h2>
                    ...
                    <form onSubmit={handleCreate} className="p-6 md:p-8 space-y-5">
                        ...
                        <div>
                            ...
                            <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className={inputClass} placeholder="staff@vortex.fitness" required />
                        </div>
                        <div>
                    ...
                    <div className="overflow-y-auto flex-1 divide-y divide-gray-100 dark:divide-gray-800/50 custom-scrollbar">
                        {isLoadingStaff ? (
                            ...
                            <div className="text-center py-16">
                                ...
                                <p className="text-gray-500 dark:text-gray-400 font-bold mb-1">No staff yet</p>
                                <p className="text-sm text-gray-400 dark:text-gray-500">Add someone using the form.</p>
                            </div>
                        ) : (
                            staffList.map(staff => {
                                const isEditing = editingRate === staff.id;
                                const thisStatus = rateStatus.userId === staff.id ? rateStatus : null;
                                return (
                                    <div key={staff.id} className="flex flex-col p-6 hover:bg-gray-50 dark:hover:bg-darkBg/50 transition-colors">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-full bg-olive/10 dark:bg-lightSage/10 flex items-center justify-center text-olive dark:text-lightSage font-black text-xl overflow-hidden border border-olive/20 dark:border-lightSage/20 shadow-sm flex-shrink-0">
                                                {staff.photoUrl ? <img src={staff.photoUrl} alt="" className="w-full h-full object-cover" /> : staff.firstName?.[0] || 'S'}
                                            </div>
                                            ...
                                            {isEditing ? (
                                                <div className="flex items-center gap-2 w-full">
                                                    <div className="relative flex-1">
                                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 font-bold text-xs">৳</span>
                                                        <input
                                                            type="number"
                                                            step="0.01"
                                                            min="0"
                                                            ...
                                                            placeholder="0.00"
                                                            className="pl-7 pr-3 py-2 w-full text-sm border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-darkBg text-gray-900 dark:text-cream focus:outline-none focus:ring-2 focus:ring-olive dark:focus:ring-lightSage"
                                                            autoFocus
                                                        />
                                                    </div>
                                                    <button
                                                        ...
                                                        disabled={savingRate}
                                                        className="bg-olive text-white dark:bg-lightSage dark:text-darkBg text-xs font-black px-4 py-2 rounded-xl hover:bg-olive/90"
                                                    >
                                                        {savingRate ? '...' : 'Save'}
                                                    </button>
                                                    <button
                                                ...
                                                <div className="flex items-center justify-between w-full">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-sm font-black text-gray-900 dark:text-white bg-gray-100 dark:bg-darkBg px-2 py-1 rounded-lg border border-gray-200 dark:border-gray-700">
                                                            ৳{Number(staff.hourlyRate || 0).toLocaleString()}/hr
                                                        </span>
                                                        {thisStatus && (
                                                    ...
                                                    <button
                                                        onClick={() => { setEditingRate(staff.id); setNewRate(staff.hourlyRate || ''); }}
                                                        className="text-[10px] font-black bg-gray-100 dark:bg-gray-800 hover:bg-olive hover:text-white dark:hover:bg-lightSage dark:hover:text-darkBg text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg transition-all"
                                                    >
                                                        ✏️ Set Rate
                                                    </button>
                                                </div>
