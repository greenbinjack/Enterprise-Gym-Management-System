...
export default function TrainerProfile() {
    ...
    const token = localStorage.getItem('token'); // Trainers need their token

    useEffect(() => {
        const fetchProfile = async () => {
            try {
                // Ensure we pass the token, even if fetching Profile.
                const res = await axios.get(`http://localhost:8080/api/trainer/${userId}/profile`, {
    ...
    }, [userId, token, currentUser?.email]);
    ...
    const handleSave = async (e) => {
        ...
        try {
            const res = await axios.put(`http://localhost:8080/api/trainer/${currentUser.id}/profile`, formData, {
                headers: {
